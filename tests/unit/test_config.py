...
def test_config_round_trip(tmp_path):
    ...
    assert loaded.execution.provider.model == "gpt-5.4"
...
def test_config_round_trip_includes_review_provider_override(tmp_path):
    ...
    assert loaded.execution.backend == "provider"
    assert loaded.execution.provider is not None
    assert loaded.execution.provider.review_model == "gpt-5.4-mini"
...
def test_config_loads_legacy_config_without_provider_block(tmp_path):
    ...
    assert loaded.execution.backend == "provider"
    assert loaded.execution.provider is None
    assert loaded.execution.agent is None
    assert loaded.provider is None
    assert loaded.agents is None


def test_config_loads_legacy_top_level_provider_block_into_execution_provider(tmp_path):
    path = tmp_path / ".pizhi" / "config.yaml"
    path.parent.mkdir(parents=True)
    path.write_text(
        "\n".join(
    ...
    assert loaded.execution.backend == "provider"
    assert loaded.execution.provider is not None
    assert loaded.execution.provider.provider == "openai_compatible"
    assert loaded.execution.provider.review_model == "gpt-5.4-mini"
    assert loaded.provider is not None
    assert loaded.provider.model == "gpt-5.4"
...
def test_save_config_omits_provider_block_when_missing(tmp_path):
    ...
    assert "execution:" not in saved_text
